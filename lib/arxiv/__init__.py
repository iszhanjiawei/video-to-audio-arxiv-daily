...
class Client(object):
    ...
    def _results(self, search: Search, offset: int = 0) -> Generator[Result, None, None]:
        total_needed = search.max_results if search.max_results is not None else float('inf')
        fetched = 0
        total_results = float('inf')  # 初始化为无限大

        while fetched < total_needed:
            # 计算本次请求需要获取的数量
            batch_size = min(self.page_size, total_needed - fetched)
            if batch_size <= 0:
                break

            page_url = self._format_url(search, offset + fetched, batch_size)

            try:
                feed = self._parse_feed(
                    page_url,
                    first_page=(fetched == 0),
                    is_retry=(fetched > 0)  # 添加重试标记
                )
                # hello
                # 如果是第一页，获取总结果数
                if fetched == 0:
                    total_results = int(feed.feed.opensearch_totalresults)
                    logger.info("Total results available: %d", total_results)

                if not feed.entries:
                    logger.info("Got empty page; stopping generation")
                    break
                ...
                # 检查是否还有更多结果
                if offset + fetched >= total_results:
                    logger.info("Reached end of available results")
                    break

            except (UnexpectedEmptyPageError, HTTPError) as e:
                logger.warning("Error fetching page at offset %d: %s", offset + fetched, e)

                # 特殊处理：跳过空页继续下一页
                if isinstance(e, UnexpectedEmptyPageError):
                    logger.info("Skipping empty page at offset %d", offset + fetched)
                    fetched += batch_size  # 跳过这一页
                    continue
                ...
                # 如果错误发生在第一页，无法确定总结果数，直接终止
                if fetched == 0:
                    logger.error("Failed to fetch first page, aborting")
                    break

                # 跳过错误页继续下一页
                logger.info("Skipping page at offset %d due to error", offset + fetched)
                fetched += batch_size
    ...
    def _parse_feed(
            ...
            is_retry: bool = False,  # 新增参数，表示是否是重试请求
            _try_index: int = 0
    ...
    def __try_parse_feed(
        ...
        # 如果是重试请求，增加额外的延迟
        if is_retry:
            extra_delay = min(5.0, 2.0 * try_index)  # 指数退避策略
            logger.info("Adding extra delay for retry: %f seconds", extra_delay)
